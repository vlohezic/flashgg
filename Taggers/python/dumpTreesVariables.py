...
ttHLeptonic_variables = ["leadPrompt           := leadPrompt",
                         ...
                         "elecCh_1             := ?(electrons.size>0)? electrons.at(0).charge : -999",
                         ...
                         "muCh_1               := ?(muons.size>0)? muons.at(0).charge : -999",
                         ...
                         #"fwdjetPt_1           := ?(Jets_EtaSorted.size>0)? Jets_EtaSorted.at(0).pt : -999",
                         #"fwdjetEta_1          := ?(Jets_EtaSorted.size>0)? Jets_EtaSorted.at(0).eta: -999.",
                         #"fwdjetPhi_1          := ?(Jets_EtaSorted.size>0)? Jets_EtaSorted.at(0).phi: -999.",
                         #"fwdjetBtag_1         := ?(Jets_EtaSorted.size>0)? (Jets_EtaSorted.at(0).bDiscriminator('pfDeepCSVJetTags:probb') + Jets_EtaSorted.at(0).bDiscriminator('pfDeepCSVJetTags:probbb')) : -999",
...
ttHHadronic_variables = ["nJets                := nJet",
                         "nJetsBm              := nBMedium",
                         "nJetsBl              := nBLoose",
                         "nJetsBt              := nBTight",
                         "leadJetPt            := leadJetPt",
                         ...
                         "btag_7               := ?(jetBTagValVec.size>6)? jetBTagValVec.at(6) : -9",
                         ...
                         "btag_8               := ?(jetBTagValVec.size>7)? jetBTagValVec.at(7) : -9",
                         "MET                  := met.getCorPt",
                         "METPx                := met.getCorPx",
                         "METPy                := met.getCorPy",
                         "METPhi               := met.phi",
